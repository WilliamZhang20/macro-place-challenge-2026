#!/usr/bin/env python3
"""Run DREAMPlace diagnostics over IBM benchmark/config sweeps."""
...
RUNNER = REPO_ROOT / "scripts" / "run_dreamplace_diagnostics.py"
DEFAULT_BENCHMARKS = ("ibm01", "ibm03", "ibm09")
DEFAULT_CONFIGS = ("0.76:200", "0.82:200")
IBM_BENCHMARKS = (
    "ibm01",
    "ibm02",
    "ibm03",
...
def main() -> int:
    ...
    for benchmark in args.benchmark:
        ...
        cmd = [
            ...
            str(out_path),
            "--dreamplace-root",
            str(args.dreamplace_root),
            "--preset",
            args.preset,
            "--soft-macro-mode",
            args.soft_macro_mode,
        ]
        for config in args.config:
            cmd.extend(["--config", config])
        for alpha in args.blend_alpha:
            cmd.extend(["--blend-alpha", str(alpha)])
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep DREAMPlace diagnostics.")
    ...
    parser.add_argument(
        "--work-root",
        type=Path,
        default=Path("/tmp/macro_place_dreamplace_sweep_work"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/tmp/macro_place_dreamplace_sweep"),
    )
    parser.add_argument(
        "--dreamplace-root",
        type=Path,
        default=REPO_ROOT / "external" / "DREAMPlace",
    )
    ...
    parser.add_argument("--blend-alpha", action="append", type=float, default=[])
    args = parser.parse_args()
    args.benchmark = list(IBM_BENCHMARKS) if args.all else args.benchmark or list(DEFAULT_BENCHMARKS)
...
def _print_table(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        selected = row["selected_label"] or "none"
        print(
            f"{row['benchmark']:>8} {row['baseline_proxy']:>10.6f} "
            f"{cand_s:>10} {delta_s:>10} {selected[:18]:>18}"
        )
