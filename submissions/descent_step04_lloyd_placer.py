"""
Step 4 — Lloyd-style alternation: global placement ↔ legalize with anchor pull.

After each legalization, re-run global with μ·||x − x_legal||² on movable hard
macros (μ ramped). Congestion term off until step 5.

Usage:
  uv run evaluate submissions/descent_step04_lloyd_placer.py -b ibm01
"""
...
class DescentStep04LloydPlacer:
    def __init__(self):
        self.outer_iters = 4
        self.lr = 0.045
    ...
    def place(self, benchmark: Benchmark) -> torch.Tensor:
        ...
        for outer in range(self.outer_iters):
            if outer == 0:
                ...
                inner_iters, mu_s, mu_e = 360, 0.05, 0.42 + 0.1 * float(outer - 1)
                anchor = cur.clone()
